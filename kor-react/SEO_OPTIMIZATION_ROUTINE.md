...
| TASK-015 | internal-linking| Under-linked: `group-ride-tips` had only 1 incoming `related[]` reference (from pillar `how-to-plan-a-bike-ride`). Fixed: added `group-ride-tips` to the `related[]` array of `strava-tips-for-cyclists` (both `articlesIndex.ts` and markdown frontmatter) — `group-ride-tips` already links out to `strava-tips-for-cyclists`, and `strava-tips-for-cyclists`'s "Find Clubs" section already covers finding a group ride via Strava Clubs, making the reciprocal link a natural fit — plus a contextual inline link in that section pointing readers to group-ride etiquette before their first ride. `npm run build` passes. | in_review | 2026-08-18 |
