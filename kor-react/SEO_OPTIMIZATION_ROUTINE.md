...
| TASK-015 | internal-linking| Under-linked: `group-ride-tips` had only 1 incoming `related[]` reference (from pillar `how-to-plan-a-bike-ride`). Fixed: added `group-ride-tips` to the `related[]` array of `strava-tips-for-cyclists` (both `articlesIndex.ts` and markdown frontmatter) — `strava-tips-for-cyclists`'s own body already discusses finding a club ride via Strava Clubs, making the reciprocal link a natural fit — plus a contextual inline link in its "Find Clubs That Match How You Actually Ride" section pointing first-time club riders to group-ride etiquette and hand signals before their first outing. `npm run build` passes. | in_review | 2026-07-24 |
