...
| TASK-015 | internal-linking| Under-linked: `group-ride-tips` had only 1 incoming `related[]` reference (from pillar `how-to-plan-a-bike-ride`). Fixed: added `group-ride-tips` to the `related[]` array of `strava-tips-for-cyclists` (both `articlesIndex.ts` and markdown frontmatter) — `group-ride-tips` already links out to `strava-tips-for-cyclists` for finding a Strava club, and `strava-tips-for-cyclists`'s own "Find Clubs That Match How You Actually Ride" section already covers group rides, making the reciprocal link a natural fit — plus a contextual inline link in that section pointing readers to group-ride etiquette before their first club ride. `npm run build` passes. | in_progress | 2026-07-24 |
