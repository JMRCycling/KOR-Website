...
| TASK-015 | internal-linking| Under-linked: `group-ride-tips` had only 1 incoming `related[]` reference (from pillar `how-to-plan-a-bike-ride`). Fixed: added `group-ride-tips` to the `related[]` array of `strava-tips-for-cyclists` (both `articlesIndex.ts` and markdown frontmatter) — `group-ride-tips` already links out to `strava-tips-for-cyclists` (added in TASK-010) for finding a club/pace group, making the reciprocal link a natural fit — plus a contextual inline link in `strava-tips-for-cyclists`'s "Find Clubs That Match How You Actually Ride" section pointing riders to group-ride etiquette before their first club outing. `npm run build` passes. | in_review | 2026-07-24 |
